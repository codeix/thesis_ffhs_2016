...
Im Anhang zur Thesis sind die Daten für jedes Board auch in tabellarischer Form aufgeführt. Die Werte \texttt{BM1}, \texttt{BM2} und \texttt{BM3} bezeichnen den Median des Stromverbrauchs des jeweiligen Benchmarks, der ebenso unterhalb der Diagramme dargestellt ist. Gleich verhält es sich für die exakte Durchlaufszeit \texttt{T1}, \texttt{T2}, und \texttt{T3} der Benchmarks. Die Kolonne \texttt{Med BM} bezeichnet den Median aller drei Benchmarks. Die Berechnung erfolgte mit allen Daten von den drei Ausführungen eines Benchmarks, wobei die ersten fünf Sekunden abgeschnitten worden sind. So bilden die Kolonnen \texttt{Med BM} und \texttt{Med T} den Durchschnittswert der drei Ausführungen eines Benchmarks ab. Diese wurden sodann für alle weiteren Berechnungen verwendet. Die zwei letzten Kolonnen zeigen den Leistungs- und Energieverbrauch.
\par
Bei der Ausführung der Messung wurden der Benchmark und das Messgerät gleichzeitig gestartet. Weil das Messgerät eine gewisse Anlaufzeit braucht, bis es exakte Resultate liefern kann, generieren die ersten Messpunkte eher ungenaue Werte. Hinzu kommt, dass auf dem Board eingebaute Kondensatoren die Stromanstieg verzögern. Wie vorgehend dargelegt, wurde aus diesen Gründen für die Berechnung der Durchschnittswerte eine eher grosszügige Zeitspanne von fünf Sekunden  nicht berücksichtigt. Dieses Vorgehen erweist sich als korrekt, da die Resultate zeigen, dass sich der Stromverbrauch, abgesehen von dieser Schwankung zu Beginn der Messung, ansonsten linear verhält.
